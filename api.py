...
def _serialize_payload(payload: ErrorPayload) -> dict[str, Any]:
    """Return a plain dictionary from a Pydantic model (v1 or v2)."""
...
try:
    ...
    curriculum_generator = None  # type: ignore[assignment]
...
async def request_validation_handler(_, exc: RequestValidationError):
    return JSONResponse(
        ...
        content=_serialize_payload(
            ErrorPayload(detail="Invalid request payload.", code="VALIDATION_ERROR")
        ),
    )
...
async def unhandled_handler(_, exc: Exception):
    ...
    return JSONResponse(
        status_code=500,
        content=_serialize_payload(ErrorPayload(detail="Internal server error.", code="UNHANDLED_ERROR")),
    )
