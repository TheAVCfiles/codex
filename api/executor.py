...
def decide(account_equity: float, p_rain: float, p_sun: float) -> Decision:
    brier = _latest_brier()
    if brier is not None and brier > 0.25:
        return Decision("HOLD", p_rain, p_sun, 0.0, "kill-switch: brier worse than naive")

    if p_rain >= 0.70:
        notional = size_notional(account_equity, p_rain, max_pct=float(CFG["max_position_pct"]))
        return Decision("BUY", p_rain, p_sun, notional, "rain window high confidence")
    if p_sun >= 0.70:
        return Decision("SELL", p_rain, p_sun, 0.0, "sun window high confidence")
    return Decision("HOLD", p_rain, p_sun, 0.0, "no high-confidence window")
...
def run(account_equity: float = 100_000, api_base: str = "http://localhost:8000") -> dict[str, Any]:
    ...
    if not grid:
        decision = Decision("HOLD", 0.5, 0.5, 0.0, "missing forecast grid")
    else:
        first = grid[0]
        decision = decide(account_equity, float(first.get("rain", 0.5)), float(first.get("sun", 0.5)))
