...
def sanitize(text: str) -> str:
    ...
    for phrase in banned:
        ...
        text = text.replace(phrase.title(), "[redacted]")
...
def main() -> None:
    ...
    for i, musing in enumerate(musings):
        if musing.get("release_state") not in ("public_now", "gated_preview"):
            continue
        slug = f"demo-{i + 1}.html"
        html = build_page(musing.get("title", "Untitled"), sanitize(musing.get("content", "")))
