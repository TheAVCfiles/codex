...
<body>
  <div class=\"wrap\">
    ...
    <div class=\"card\">
      ...
      <ul>
        <li>Release score: {stats["release_score"]}</li>
        <li>Market score: {stats["market_score"]}</li>
        <li>Leak risk score: {stats["leak_risk_score"]}</li>
      </ul>
    </div>
    <div class=\"card\">
      <h2>Next Step</h2>
...
def main() -> None:
    ...
    for i, m in enumerate(musings):
        if m["release_state"] != "gated_preview":
            continue
        (OUTDIR / f"room-{i + 1}.html").write_text(build_room(m["title"], m))
