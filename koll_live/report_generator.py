...
def _narrative(metrics: dict, best: dict | None, wf_df: pd.DataFrame | None, transits: pd.DataFrame | None) -> str:
    lines: list[str] = []
    sr = metrics.get("sharpe", 0.0)
    dd = metrics.get("max_drawdown", 0.0)
    cr = metrics.get("cum_return", 0.0)

    if sr > 1.5:
        lines.append("Performance was robust with Sharpe above 1.5, indicating consistent risk-adjusted returns.")
    elif sr > 1.0:
        lines.append("Performance was solid with Sharpe above 1.0; risk-adjusted results were positive across regimes.")
    else:
        lines.append("Sharpe below 1.0 suggests signal quality was inconsistent; consider tightening thresholds or cooldown.")

    if dd < -0.2:
        lines.append("Max drawdown exceeded 20%, implying the breaker may need earlier engagement or lower position sizing.")
    else:
        lines.append("Drawdown remained contained, consistent with a grounded risk posture.")

    if best:
        lines.append(
            f"Best parameters favored low_q={best.get('low_q')} high_q={best.get('high_q')} cooldown={best.get('cooldown')}—a mid-band entry with fast exits."
        )

    if wf_df is not None and "test_sharpe" in wf_df.columns:
        mean_wf = wf_df["test_sharpe"].mean()
        lines.append(f"Walk-forward mean test Sharpe: {mean_wf:.2f}, supporting generalization beyond the training slice.")

    if transits is not None:
        lines.append(
            "Astrology overlay: report aligned results with provided transit windows; use these labels for narrative context only."
        )
...
def generate_report(
    ...
    narrative_txt = _narrative(metrics, top_params, wf_df, transits) if narrative else ""
