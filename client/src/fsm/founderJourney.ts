...
export function nextState(current: FounderState): FounderState {
  const idx = founderSteps.findIndex((s) => s.state === current);
  if (idx < founderSteps.length - 1) return founderSteps[idx + 1].state;
  return current;
}
...
export function getStepIndex(state: FounderState): number {
  const idx = founderSteps.findIndex((s) => s.state === state);
  return idx < 0 ? 0 : idx;
}
